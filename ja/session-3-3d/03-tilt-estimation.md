...
> **第2回とのつながり**：第2回では「傾き $`\theta_b`$ はセンサで測れる」と仮定して制御を組み立てました。その“測る中身”がまさにこのページです。3次元では、重力の向きからピッチ・ロールを**計算して**手に入れます（式36）。
...
- 6個も使うのはなぜ？（やさしく）
- （くわしく）$`^{B}\hat{g}`$ から $`\hat\beta,\hat\gamma`$ を出すのに使う関数は？
- （くわしく）ヨー $`\alpha`$ が重力から分からないのはなぜ？

次は [4. 相補フィルタで合成](./04-complementary-filter.md)
