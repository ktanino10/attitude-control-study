...
> **式16とのつながり**：$`\bar{\beta}_{k-1} + \dot{\hat{\beta}}_k\,\Delta t`$ は「前回の値＋角速度×時間」。これは第2回の離散化 $`\mathbf{x}[k{+}1]\approx\mathbf{x}[k]+\Delta t\,\dot{\mathbf{x}}[k]`$（式16）とまったく同じ**前進オイラー**の“1コマ先予測”です。フィルタの中でも、同じ積分の考え方が働いています。
...
- （くわしく）$`\kappa=0`$ にすると、どちらのセンサだけになる？
- （くわしく）「相補（たすと1）」とは、どの2つのフィルタの関係？

次は [5. 3次元の運動方程式（結果の概観）](./05-3d-equations.md)
