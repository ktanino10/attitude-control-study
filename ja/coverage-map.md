...
送っていただいた写真＝**トランジスタ技術 2020年6月号 第10章「XYZ自由自在！3軸姿勢制御モジュール誕生」（茂渡 修平／JAXA, pp.110–114）** の内容を、
...
- 元記事は**概要記事**なので、細かいセンサ推定（カルマン）と制御計算（トルク・PID）は、この座学では**第2回・第3回**でしっかり扱います。
- 第1回（今回）は、写真に出てくる用語と全体像を**もれなく**押さえるのが目的です。上の表のとおり、写真の各項目はどこかで必ず解説しています。
